...
namespace WarpDB2
{
    public class WarpDB2 : AOPluginEntry
    {
        ...
        public override void Run(string pluginDir)
        {
            try
            {
                _settings = new Settings("WarpDB2");
                PluginDir = pluginDir;

                Config = Config.Load($"{Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData)}\\AOSharp\\WarpDB2\\{Game.ClientInst}\\Config.json");
                ...
                SettingsController.RegisterSettingsWindow("WarpDB2", pluginDir + "\\UI\\WarpDB2SettingWindow.xml", _settings);

                _stateMachine = new StateMachine(new IdleState());

                Chat.WriteLine("WarpDB2 Loaded!");
                Chat.WriteLine("/db2 for settings.");
                ...
                Chat.RegisterCommand("buddy", WarpDB2Command);
        ...
        {
            _infoWindow = Window.CreateFromXml("Info", PluginDir + "\\UI\\WarpDB2InfoView.xml",
        ...
        private void OnUpdate(object s, float deltaTime)
        {
            ...
            if (SettingsController.settingsWindow != null && SettingsController.settingsWindow.IsValid)
            {
                ...
                if (SettingsController.settingsWindow.FindView("WarpDB2InfoView", out Button infoView))
                {
        ...
        private void WarpDB2Command(string command, string[] param, ChatWindow chatWindow)
        {
            try
            {
                if (param.Length < 1)
