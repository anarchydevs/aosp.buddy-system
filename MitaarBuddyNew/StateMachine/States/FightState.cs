...
namespace MitaarBuddy
{
    public class FightState : IState
    {
        ...
        public void Tick()
        {
            ...
            {
                if (MitaarBuddy._leaderPos != Vector3.Zero && DynelManager.LocalPlayer.Position.DistanceFrom(MitaarBuddy._leaderPos) > 1.2f
            ...
            else if (DynelManager.LocalPlayer.Identity == MitaarBuddy.Leader 
                && MitaarBuddy.DifficultySelection.Normal == (MitaarBuddy.DifficultySelection)MitaarBuddy._settings["DifficultySelection"].AsInt32())
                HandleTaunting(_target);
            else if (MitaarBuddy.DifficultySelection.Medium == (MitaarBuddy.DifficultySelection)MitaarBuddy._settings["DifficultySelection"].AsInt32())
                HandlePathing(_target);
        }
        ...
        {
            if (MitaarBuddy.NavMeshMovementController.IsNavigating && target.IsInLineOfSight)
            {
            ...
            if (target.Position.DistanceFrom(DynelManager.LocalPlayer.Position) > 4f
                ...
                || GetWieldedWeapons(DynelManager.LocalPlayer).HasFlag(CharacterWieldedWeapon.MartialArts)))
            {
                MitaarBuddy.NavMeshMovementController.SetNavMeshDestination(target.Position);
            }
        ...
        public static void HandleTaunting(SimpleChar target)
        {
            if (_aggToolCounter >= 2)
            {
                ...
                if (_attackTimeout >= 1)
                {
                    MitaarBuddy.NavMeshMovementController.SetDestination(target.Position);
                    _attackTimeout = 0;
                    _aggToolCounter = 0;
                    return;
                }
