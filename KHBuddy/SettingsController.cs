...
namespace KHBuddy
{
    public static class SettingsController
    {
        ...
        private static void RegisterChatCommandIfNotRegistered()
        {
            ...
            {
                Chat.RegisterCommand("khbuddy", (string command, string[] param, ChatWindow chatWindow) =>
                {
                    try
                    {
                        Config = Config.Load($"{Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData)}\\AOSharp\\KnowsMods\\KHBuddy\\{Game.ClientInst}\\Config.json");

                        settingsWindow = Window.Create(new Rect(50, 50, 300, 300), "KHBuddy", "Settings", WindowStyle.Default, WindowFlags.AutoScale);
