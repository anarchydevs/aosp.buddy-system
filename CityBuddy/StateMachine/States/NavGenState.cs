...
namespace CityBuddy
{
    ...
    {
        private string filePath = $"{CityBuddy.PluginDir}\\Navmeshes\\{Playfield.ModelIdentity.Instance}.Navmesh";
        ...
        public IState GetNextState()
        {
            if (CityBuddy._settings["Enable"].AsBool())
            {
                if (NavmeshFileExists)
                {
                    ...
                    {
                        CityBuddy.IPCChannel.Broadcast(new EnterMessage());
        ...
        public void NavGen()
        {
            ...
            NavmeshGenerator.BakeAsync(NMGenParams).ContinueWith(async (task) =>
            {
                if (task.IsFaulted)
                {
                    ...
                    CityBuddy.IPCChannel.Broadcast(new ClearSelectedMemberMessage());
                    MovementController.Instance.SetMovement(MovementAction.BackwardStart);
                }
                else
                {
        ...
        {
            string navMeshFolderPath = $"{CityBuddy.PluginDir}\\NavMeshes\\";
