...
namespace LeBuddy
{
    public static class SettingsController
    {
        ...
        private static void RegisterChatCommandIfNotRegistered()
        {
            ...
            {
                Chat.RegisterCommand("le", (string command, string[] param, ChatWindow chatWindow) =>
                {
                    try
                    {
                        ...
                        settingsWindow = Window.Create(new Rect(50, 50, 300, 300), "LeBuddy", "Settings", WindowStyle.Default, WindowFlags.AutoScale);

                        if (settingsWindow != null && !settingsWindow.IsVisible)
                        {
                            AppendSettingsTab("LeBuddy", settingsWindow);
