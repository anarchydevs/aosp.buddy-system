...
namespace LeBuddy
{
    public static class Extensions
    {
        ...
        {
            return DynelManager.LocalPlayer.Position.DistanceFrom(Constants._unicornRecruiter) < 8.5f;
        }

        public static bool HasDied()
        {
            return DynelManager.LocalPlayer.Position.DistanceFrom(Constants._reclaim) < 5;
        }
        ...
        {
            if (LeBuddy.NavMeshMovementController.IsNavigating && target?.IsInLineOfSight == true)
            {
            ...
            if (target?.Position.DistanceFrom(DynelManager.LocalPlayer.Position) > 4f
                && (GetWieldedWeapons(DynelManager.LocalPlayer).HasFlag(CharacterWieldedWeapon.Melee)
                || GetWieldedWeapons(DynelManager.LocalPlayer).HasFlag(CharacterWieldedWeapon.MartialArts)))
                LeBuddy.NavMeshMovementController.SetNavMeshDestination((Vector3)target?.Position);
